...
_create_prvkey(struct sc_pkcs15_card * p15card, unsigned file_id)
{
	...
	{		
		...
		guid[guid_len] = 0;
		rv = sc_aux_data_set_md_guid(ctx, pinfo->aux_data, guid);
...
_create_data_object(struct sc_pkcs15_card * p15card, const char* path, unsigned file_id)
{
	...
	// // TEMP File is is private
	// //if (rv == SC_ERROR_SECURITY_STATUS_NOT_SATISFIED) {
	// if (rv != SC_SUCCESS) {
	// 	LOG_TEST_RET(ctx, rv, "Error while getting file content.");

	// 	struct sc_file *file = NULL;
	// 	int rvs = sc_select_file(p15card->card, &info.path, &file);
	// 	LOG_TEST_RET(ctx, rvs, "Error while selecting file.");
	...
	// // TEMP
	// char path_buffer[SC_MAX_PATH_STRING_SIZE + SC_MAX_AID_STRING_SIZE] = {0};
	// sc_path_print(path_buffer, sizeof(path_buffer), &info.path);
	// if(!strcmp(path_buffer, "3f0030003001867f"))
	// {
	// 	DebugBreak();
	// }
...
_parse_fs_data(struct sc_pkcs15_card * p15card)
{
	...
	// If not add folder manually, it will added automatically and the "enumerated" flag will be set to true
	// and laser_parse_df will not be called. 
	...
	for (df=0; df_paths[df]; df++)   {
		...
		for (ii=0; ii<count; ii++)   {
			...
			case LASER_TYPE_KX_DATA:
				...
				if(strcmp(PATH_PRIVATEDIR, df_paths[df])) // TEMP P15 DF RELOAD PRIVATE - not load private data-objects
				{
		...
		for (offs = 0, rec_num=0; offs < data->data_len; rec_num++)   {
			...
			if (rec->keysize_sign || rec->keysize_keyexchange)   {
				...
				for (ii=0; ii<prkeys_num; ii++)   {
					...
					sc_log(ctx, "Key(ref:0x%lX)", info->key_reference);
					if (rec_num == key_idx) {
						...
						guid[guid_len] = 0;
						rv = sc_aux_data_set_md_guid(ctx, info->aux_data, guid);
						LOG_TEST_RET(ctx, rv, "Cannot set MD CMAP Guid");
...
sc_pkcs15emu_laser_init(struct sc_pkcs15_card * p15card)
{
	...
	// set_string(&p15card->tokeninfo->label, "JaCarta Laser");
	// set_string(&p15card->tokeninfo->manufacturer_id, "JaCarta Laser");
	// /* set dummy until we found serial number */
	// set_string(&p15card->tokeninfo->serial_number, "00000000");

	// p15card->tokeninfo->version = 0;
	// p15card->tokeninfo->flags = CKF_TOKEN_INITIALIZED | CKF_USER_PIN_INITIALIZED;
	...
	p15card->card->version.hw_major = *(buf + 140);
	p15card->card->version.hw_minor = *(buf + 141);
	p15card->card->version.fw_major = *(buf + 142);
	p15card->card->version.fw_minor = *(buf + 143);
