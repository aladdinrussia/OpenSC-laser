...
static struct sc_atr_table laser_known_atrs[] = {
	...
	{ "3B:80:80:01:01", NULL, // Old NFC-cards P60/P71 with empty HistBytes
		"LASER NFC", SC_CARD_TYPE_ATHENA_LASER, 0, NULL },
	{ "3B:8C:80:01:80:73:C8:21:13:66:01:06:11:59:00:01:2C", NULL, // New NFC-cards P60/P71
		"LASER NFC", SC_CARD_TYPE_ATHENA_LASER, 0, NULL },
...
laser_generate_key(struct sc_card *card, struct sc_cardctl_laser_genkey *args)
{
	struct sc_context *ctx = card->ctx;
	struct sc_apdu apdu;
	unsigned char sbuf[SC_MAX_APDU_BUFFER_SIZE], rbuf[2*SC_MAX_APDU_BUFFER_SIZE];
	const unsigned char *ptr = NULL;
	size_t offs, taglen;
	int rv;
...
laser_sm_wrap_apdu(struct sc_card *card, struct sc_apdu *in_apdu, struct sc_apdu **out_apdu)
{
	...
	if (sm_level == 3)   {
		...
		if (apdu->data && apdu->datalen)  {
			...
			rv = sm_encrypt_des_cbc3(ctx, sess->session_enc, apdu->data, apdu->datalen, &val, &val_len, 0/*SC_SM_PADDING_MANDATORY*/);
